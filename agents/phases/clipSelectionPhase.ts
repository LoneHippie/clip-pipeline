  ...
  clips: z
    .array(
      z.object({
        ...
        hook:           z.string().describe('Opening sentence/phrase that grabs attention in the first 3 seconds'),
        viralityReason: z.string().describe('One sentence explaining why this clip works as a standalone short'),
      })
    )
    .min(1)
    .max(5),
...
export async function runClipSelectionPhase(
  ...
  onChunk?.({ type: 'phase-start', phase: 'clip-selection', text: 'Starting clip selection...' });
  ...
  onChunk?.({ type: 'tool-call', phase: 'clip-selection', toolName: 'uploadVideoToGemini', input: { videoPath } });
  const uploadResponse = await fileManager.uploadFile(videoPath, {
    mimeType,
    displayName: path.basename(videoPath),
  });
  onChunk?.({ type: 'tool-result', phase: 'clip-selection', output: { fileName: uploadResponse.file.name } });
  ...
  onChunk?.({ type: 'tool-call', phase: 'clip-selection', toolName: 'selectClipsViaGemini', input: { fileUri: file.uri } });
...
- Return start/end as MM:SS only — do NOT return word-level timestamps`,
  ...
  onChunk?.({ type: 'tool-result', phase: 'clip-selection', output: { clipCount: object.clips.length } });

  // Clean up the Gemini-hosted file — it auto-expires in 48h but we delete proactively
  await fileManager.deleteFile(uploadResponse.file.name).catch(() => {
    console.warn(`[clipSelectionPhase] Failed to delete Gemini file: ${uploadResponse.file.name}`);
  });
  ...
  const clips = object.clips.map(clip => {
    ...
    if (duration < 30 || duration > 120) {
      throw new Error(`Clip "${clip.title}" has invalid duration: ${duration}s (expected 30–120s)`);
    }
  ...
  onChunk?.({ type: 'phase-complete', phase: 'clip-selection', text: `Selected ${clips.length} clips.` });
  return clips;
}

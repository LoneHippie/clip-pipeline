...
const MetadataSchema = z.object({
  tiktok: z.object({
    caption:  z.string().describe('Hook first line. Max 150 chars. No hashtags in body.'),
    hashtags: z.array(z.string()).describe('3–5 hashtags, each prefixed with #'),
  }),
  instagram: z.object({
    caption:  z.string().describe('2–3 sentences. Max 300 chars. End with a question.'),
    hashtags: z.array(z.string()).describe('10–15 hashtags, each prefixed with #'),
  }),
  youtubeShorts: z.object({
    title:       z.string().describe('SEO title, max 60 chars, front-load keyword'),
    description: z.string().describe('2–3 sentences with keywords + full video CTA. Max 200 chars.'),
...
export async function runMetadataPhase(
  ...
  onChunk?.({ type: 'phase-start', phase: 'metadata', text: 'Generating platform metadata...' });
  ...
  onChunk?.({ type: 'tool-call', phase: 'metadata', toolName: 'generatePlatformMetadata', input: { title } });
...
Write copy that feels native to each platform. TikTok: punchy and direct. Instagram: conversational with a CTA question. YouTube Shorts: SEO-focused title.`,
  ...
  onChunk?.({ type: 'tool-result', phase: 'metadata', output: { platforms: ['tiktok', 'instagram', 'youtubeShorts'] } });
  onChunk?.({ type: 'phase-complete', phase: 'metadata', text: 'Platform metadata generated.' });

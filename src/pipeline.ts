...
export async function runPipeline(jobId: string, localPath: string, db: Db): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < clips.length; i++) {
      ...
      const insertResult = db.insertClip.run(jobId, clip.title, clip.startSec, clip.endSec);
      ...
      try {
        ...
        const words = await transcribeClipFromVideo(
          localPath, clip.startSec, clip.endSec, TMP_DIR, slug,
          chunk => { if (chunk.type === 'phase-complete') log(chunk.text); },
        );
